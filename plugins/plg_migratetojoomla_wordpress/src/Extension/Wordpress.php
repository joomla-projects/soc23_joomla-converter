...
final class Wordpress extends CMSPlugin implements SubscriberInterface
{
    ...
    public function storePrimarykey()
    {
        ...
        foreach ($importstring as $key => $value) {
            Loghelper::writelog("Import string : " . $key . "   :   " . $value, "success");
        }
        // Specify the table name
        $tablePrefix = $data['dbtableprefix'];
        $tablesmap = [
            'user' => ["_users", "ID",""],
            'tag' => ["_term_taxonomy", "term_id" , $db->quoteName('taxonomy') . '=' . $db->q('post_tag')],
            "categories" => ["_term_taxonomy", "term_id" , $db->quoteName('taxonomy') . '=' . $db->q('category')],
            // "menu_types"=>"",
            // "menu"=>"",
            // "content"
        ];
        ...
        foreach ($tablesmap as $table => $tableinfo) {
            // Loghelper::writelog("key class call".$table , "success");
            if (@in_array($table, $importstring)) {

                $tableName = rtrim($tablePrefix, '_') . $tableinfo[0];
                $query = $db->getQuery(true)
                    ->select($db->quoteName($tableinfo[1]))
                    ->from($db->quoteName($tableName));
                if(!empty($tableinfo[2])) {
                    $query = $db->getQuery(true)
                    ->select($db->quoteName($tableinfo[1]))
                    ->from($db->quoteName($tableName))
                    ->where($tableinfo[2]);
    ...
    public function importUser(EventInterface $event)
    {
        ...
        // user group
        $usergroupinfo = Factory::getApplication()->getUserState('com_migratetojoomla.parameter', [])["frameworkparams"]["usergroup"];
        LogHelper::writeLog("User group :  ".$usergroupinfo , "success");
        ...
        try {
            ...
            $usergroup->group_id = intval($usergroupinfo);
